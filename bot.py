...
if not BOT_TOKEN or not SESSION_STRING:
    raise ValueError('BOT_TOKEN и SESSION_STRING должны быть заданы в переменных окружения')

# При использовании StringSession api_id и api_hash можно не указывать (0 и '').
client = TelegramClient(StringSession(SESSION_STRING), 0, '').start(bot_token=BOT_TOKEN)
logging.basicConfig(level=logging.INFO)

# ============================================================
# 1. ПОЛНЫЙ ПРОФИЛЬ (Telegram)
# ============================================================
...
# ============================================================
# 3. ПРОВЕРКА В СКАМ-БАЗАХ (SmartUserInfo)
# ============================================================
...
async def main():
    await client.start()
    logging.info('Бот запущен на Render. Команда: /check @username')
