...
PRICE_PER_REQUEST = int(os.environ.get('PRICE_PER_REQUEST', 1))  # в Stars или др.
...
async def buy_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Генерируем ссылку на оплату через Telegram Stars
    # Здесь можно использовать встроенный Payment API или просто дать криптоадрес
    await update.message.reply_text(
        "💳 Пополните баланс:\n"
