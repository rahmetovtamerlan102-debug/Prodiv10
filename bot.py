...
# ============================================================
# МЕТОД 1: GIFT ASSET API (список подарков)
# ============================================================
...
# ============================================================
# МЕТОД 2: GIFT ASSET API (детали подарка, поле transfers)
# ============================================================
...
# ============================================================
# МЕТОД 3: ПАРСИНГ СТРАНИЦЫ t.me/nft/
# ============================================================
...
# ============================================================
# МЕТОД 4: TON BLOCKCHAIN (toncenter.com)
# ============================================================
...
# ============================================================
# МЕТОД 5: MRKT API (неофициальный маркетплейс)
# ============================================================
async def fetch_mrkt_api(gift_id):
    url = f'https://api.mrkt.tech/v1/gifts/{gift_id}'
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=10) as resp:
                if resp.status != 200:
...
# ============================================================
# МЕТОД 6: TELEGRAM BOT API (getUserGifts) — список подарков
# ============================================================
async def get_user_gifts_telegram(user_id):
    try:
        url = f'https://api.telegram.org/bot{BOT_TOKEN}/getUserGifts'
        payload = {'user_id': user_id, 'limit': 50}
        async with aiohttp.ClientSession() as session:
        ...
        logger.error(f"getUserGifts error: {e}")
        return None

# ============================================================
# ОСНОВНАЯ ФУНКЦИЯ — ПЕРЕБОР ВСЕХ МЕТОДОВ
# ============================================================
async def get_history(user_id):
    ...
    # МЕТОД 6: Telegram Bot API (getUserGifts) — приоритетный
    gifts = await get_user_gifts_telegram(user_id)
    if gifts is None or not gifts:
        # МЕТОД 1: Gift Asset API (резерв)
    ...
    for gift in gifts:
        ...
        # МЕТОД 3: Парсинг страницы (самый надёжный)
        ...
        # МЕТОД 5: MRKT API
        mrkt_transfers = await fetch_mrkt_api(gift_id)
        if mrkt_transfers:
            for tr in mrkt_transfers:
                all_transfers.append({'gift': gift_name, 'from': tr.get('from', '?'), 'to': tr.get('to', '?'), 'timestamp': tr.get('timestamp')})
            continue
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/nft_manual <ссылка> — вручную по ссылке на подарок\n"
        "/help — справка",
        parse_mode='Markdown'
    )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Если автоматический поиск не работает, используйте:\n"
        "/nft_manual https://t.me/nft/ViceCream-299743",
        parse_mode='Markdown'
    )
...
async def nft_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if transfers is None:
        await update.message.reply_text("⚠️ Все методы получения данных не сработали. Попробуйте позже.")
        return
    if not transfers:
        await update.message.reply_text("📭 Нет истории передач для этого пользователя.")
        return
...
async def nft_manual_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("❌ Укажите ссылку на подарок: /nft_manual https://t.me/nft/ViceCream-299743")
        return
    url = context.args[0].strip()
    slug = url.split('/')[-1]
    await update.message.reply_text(f"🔍 Парсим страницу {slug}...")
